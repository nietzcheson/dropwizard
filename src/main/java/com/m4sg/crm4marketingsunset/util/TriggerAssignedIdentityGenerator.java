...
public class TriggerAssignedIdentityGenerator extends AbstractPostInsertGenerator {
    ...
    public static class Delegate extends AbstractReturningDelegate {
        ...
        protected PreparedStatement prepare(String insertSQL, org.hibernate.engine.spi.SessionImplementor session) throws SQLException {
            return session.connection().prepareStatement(insertSQL,keyColumns);
        }

        protected Serializable executeAndExtract(PreparedStatement insert, org.hibernate.engine.spi.SessionImplementor sessionImplementor) throws SQLException {
            insert.executeUpdate();
            Serializable id = IdentifierGeneratorFactory.getGeneratedIdentity(
                    insert .getGeneratedKeys(), getPersister().getIdentifierType());
